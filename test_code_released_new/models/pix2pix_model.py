...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
     ...
     #   self.I_E=Estimate().cuda()
        ...
        self.optimizer_G = torch.optim.RMSprop(self.netG.parameters(), lr=0.0002, alpha=0.99, eps=1e-8)#torch.optim.Adam(self.netG.parameters(), #                                                lr=opt.lr, betas=(0, 0.999),amsgrad=False,weight_decay=0)
        self.optimizer_GN =torch.optim.RMSprop(self.netGN.parameters(), lr=0.0002, alpha=0.99, eps=1e-8)# torch.optim.Adam(self.netGN.parameters(), #torch.optim.RMSprop(self.netGN.parameters(), lr=opt.lr)#

    #    self.optimizer_DA = torch.optim.RMSprop(self.netDA.parameters(), lr=0.0002, alpha=0.99, eps=1e-8)#torch.optim.Adam(self.netDA.parameters(), #torch.optim.RMSprop(self.netDA.parameters(), lr=opt.lr)#
    ...
    #    for optimizer in self.optimizers:
     #           self.schedulers.append(networks.get_scheduler(optimizer, opt))
        ...
        if not self.isTrain or opt.continue_train:
            self.load_network(self.netG, 'G', opt.which_epoch,self.optimizer_G)
            self.load_network(self.netGN, 'GN', opt.which_epoch,self.optimizer_GN)
...
#                    self.param_A=self.param_A.view(-1,20).float()
                    ...
                    self.fake_B = self.netGN(torch.cat([self.real_A,self.AUN],dim=1))
                    #####################ORIGINAL######################################################
                 ...
                 #   I_f=self.I_E(self.fake_B)
                    ...
                    self.fake_B_recon=self.netG(torch.cat([self.fake_B.data,AUR],dim=1))
